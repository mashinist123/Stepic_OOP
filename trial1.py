...
class Router:
    ...
    def send_data(self):
        while self.buffer:
            i = self.buffer.pop()
            if i.ip in self.all_connections:
                self.all_connections[i.ip].buffer.append(i)
...
print('buffer of router', router.buffer)

router.send_data()
print('данные после отправки из буфера роутера в сервере', sv_to.buffer)

print(len(router.buffer))
msg_lst_from = sv_from.get_data()
msg_lst_to = sv_to.get_data()
print(msg_lst_from[0].data)
print(len(msg_lst_to))
print(msg_lst_to[0].data, msg_lst_to[1].data)
...
print('all_conections', router.all_connections)
router.unlink(sv_to)
print(router.all_connections)
sv_from.send_data(Data("Hello", sv_to.get_ip()))
router.send_data()
